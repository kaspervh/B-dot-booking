class Appointment < ApplicationRecord
    ...
    work_periods.each do |work_period|
      if work_period.appointments.any?
        ...
        work_period.appointments.each do |appointment|
          ...
          start_time = convert_to_timestamp(work_period, appointment.end_time) + 15.minutes
        end
        new_appointment.push(calculate_appointment_time(start_time, work_period.end_time, work_period, service.duration))
        possible_appointment_times.push(new_appointment.flatten)
      else
  ...
  def self.convert_to_timestamp(work_period, time)
    
    if time.class == String
      return "#{work_period.end_time.to_time.to_s.split(' ')[0]} #{time}:00 +0000".to_time 
    else 
      return time
    end
  end
  ...
  def self.calculate_appointment_time(start_time, end_time, work_time, duration)
    ...
    time_between_in_minutes = (convert_to_timestamp(work_time, end_time) - convert_to_timestamp(work_time, start_time))/60
